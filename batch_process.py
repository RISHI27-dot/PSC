...
def separate_frames():
    """Move ir_*.bin and rgb_*.bin from frames/ directory into IR/ and RGB/ directories"""
    frames_path = Path('frames')

    if not frames_path.exists():
        print("No 'frames' directory found, skipping frame separation")
        return

    for dest_dir, pattern in [('IR', 'ir_*.bin'), ('RGB', 'rgb_*.bin')]:
        dest_path = Path(dest_dir)
        dest_path.mkdir(exist_ok=True)

        bin_files = sorted(frames_path.glob(pattern))
        if not bin_files:
            print(f"No {pattern} files found in frames/")
            continue

        print(f"\nMoving {len(bin_files)} {pattern} files to {dest_dir}/...")
        for f in bin_files:
...
def concatenate_txt_files():
    """Concatenate per-frame txt files into IR.txt and RGB.txt"""
    for directory in DIRS:
        dir_path = Path(directory)
        ...
        prefix = directory.lower()  # 'ir' or 'rgb'
        txt_files = sorted(dir_path.glob(f'{prefix}_*.txt'))

        if not txt_files:
            print(f"No {prefix}_*.txt files found in {directory}")
            continue

        output_path = dir_path / f'{directory}.txt'
        print(f"\nConcatenating {len(txt_files)} files into {output_path}...")

        with open(output_path, 'w') as out_f:
            for txt_file in txt_files:
                out_f.write(f"=== {txt_file.name} ===\n")
                out_f.write(txt_file.read_text())
                out_f.write("\n")
